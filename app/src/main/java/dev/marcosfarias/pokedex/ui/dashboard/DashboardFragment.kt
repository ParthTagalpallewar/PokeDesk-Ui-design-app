    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        dashboardViewModel.getPokemonById(id).observe(viewLifecycleOwner, Observer { pokemonValue ->
            pokemonValue?.let { pokemon ->
                ...
                activity?.window?.statusBarColor = PokemonColorUtil(view.context).getPokemonColor(pokemon.typeofpokemon)
                ...
                val pager = viewPager
                val tabs = tabs
                pager.adapter = ViewPagerAdapter(requireFragmentManager(), requireContext(), pokemon.id!!)
